...
| 6 | System A persistence | SQLite (`better-sqlite3`), diagram stored as one JSON document + Yjs update blob | Postgres (server dependency); flat files (no atomicity) | Matches JSON round-trip requirement; zero install |
...
## Data Flow
    ...
    canvas edit ────────────────────────────────→ core.applyDelta(IR)
                          ...
                          ┌─────────────────────────────────┤
                          ▼                                 ▼
                    Y.Doc sync ─→ peers            SQLite JSON doc
                                                            │
                                                    codegen ▼ (templates)
