...
- [x] 14.10 Dockerfile multi-stage build para ejecución sin configuración previa (`docker compose up --build`): compila el código fuente Java adentro de un contenedor Maven 3.9 + Temurin 21 (`build`) y traslada el `.jar` empaquetado a una imagen liviana JRE 21 Alpine de ejecución, eliminando el requisito de compilar previamente en la máquina anfitriona y previniendo el error `lstat /target: no such file or directory`.
- [x] 14.11 Mapeo de atributo explícito `id` a clave primaria JPA (`@Id Long id`): cuando una clase UML contiene explícitamente un atributo `id` (o `ID`, `Id`), codegen lo mapea a la clave primaria sintética de JPA en lugar de redeclararlo como campo de entidad regular, evitando colisiones de variables duplicadas (`variable id is already defined`) y métodos `getId()` con tipos incompatibles (`String cannot be converted to Long`). 84/84 pruebas pasando en `@app/codegen`.
...
- [x] 15.8 Soporte robusto de contenedores y tipos XMI 2.1: permitir <uml:Package> como contenedor raíz bajo <xmi:XMI>, múltiples raíces en arreglo, <ownedMember>, tipos sin prefijo o con prefijo xsi, y evitar falsos positivos de importación exitosa cuando se detectan 0 clases en el archivo (alerta en ImportXmiButton e i18n EN/ES).
- [x] 15.9 Resiliencia de importación XMI ante reyecciones de motor (BatchError): promoción de clasificadores destino de realización a interfaces (`kind: 'interface'`), deduplicación de miembros por clase (atributos y métodos sobrecargados), deduplicación de aristas (generalizaciones, realizaciones, dependencias) y desambiguación automática de clases importadas contra las ya existentes en el lienzo del diagrama para prevenir DuplicateClassError.
- [x] 13e.14 Delimitación visual del lienzo (hoja de diagrama estilo Enterprise Architect): componente de hoja visual delimitada renderizado mediante `ViewportPortal` en React Flow (`diagram-canvas-boundary`), con dimensiones estándar mínimas (3200 × 2200 px), expansión reactiva ante elementos remotos, sombreado exterior, etiqueta de hoja e i18n (`canvas.pageBoundary`), con `pointer-events: none` para no alterar interacciones.
- [x] 13e.15 Arrastre de nodos en tiempo real a 60/120 FPS: sincronización de `displayNodes` mediante `applyNodeChanges` en `onNodesChange` de `DiagramCanvas.tsx`, permitiendo que el elemento acompañe al cursor con fluidez absoluta mientras se arrastra, persistiendo la coordenada canónica en el Y.Doc al soltar (`onNodeDragStop`).
- [x] 13e.16 Arcos de salto en intersecciones ortogonales (Line Jumps estilo Enterprise Architect): detección de cruces ortogonales de 90 grados y reemplazo de segmentos rectos horizontales por arcos semicirculares de puente (`A 6 6 0 0 ...`) en `edgeCrossings.ts`, coordinados de forma reactiva entre aristas mediante `EdgeCrossingContext.tsx` y `useOrthogonalPathWithJumps`.
- [x] 13e.17 Unificación estética y responsividad de modales y botones: creación de `.modal-backdrop` con desenfoque de fondo y centrado absoluto, tarjetas `.modal-dialog` responsivas con sombreado y bordes redondeados (`DeltaPreviewModal` y `PhotoReviewModal`), y estilización coherente de botones primarios/secundarios y barra de herramientas en `canvas.css`.
...
- [x] 15b.7 Emit EA native diagram extension (`<xmi:Extension extender="Enterprise Architect">`) with `<diagrams><diagram>` and `<elements>` so Enterprise Architect auto-creates and renders the visual diagram node upon import.
- [x] 15b.7a Test: verify exported XMI includes Enterprise Architect extension containing diagram node, properties, and element geometry.
...
- [x] 19.1 Create external `mobile-test-client/` (Flutter, Dart SDK required; outside codegen output, excluded from A's pnpm build) (mobile:R1). Implemented at repo root `mobile-test-client/`, outside codegen templates and excluded from `pnpm-workspace.yaml`.
- [x] 19.2 Runtime-configurable backend base URL (settings screen persisted via shared_preferences); retarget without rebuild (mobile:R4). Implemented in `lib/config/api_config.dart` + `SettingsScreen` with localhost, emulator (10.0.2.2) and LAN presets, URL normalization and runtime update without rebuild. Verified with 4 unit tests (`api_config_test.dart`).
- [x] 19.3 CRUD screen: full cycle on demo entity via `http` package; backend-down ⇒ explicit connection error, no stale data shown (mobile:R2). Implemented in `Customer` model, `CustomerService` (list, get, create, update, delete) and `CrudScreen` Material 3 UI with pull-to-refresh and explicit connection error view when server is down (no stale data shown). Verified with 8 unit tests (`customer_model_test.dart`, `customer_service_test.dart`).
- [x] 19.4 Assistant screen: scripted action results + canned fallback verbatim (mobile:R3). Implemented in `AssistantRequest`/`AssistantResponse` models, `AssistantService` and `AssistantScreen` with quick-prompt chips, action badges and verbatim canned fallback rendering. Verified with 7 unit tests (`assistant_model_test.dart`, `assistant_service_test.dart`).
- [x] 19.5 Scope check: only endpoint config + demo-entity CRUD + assistant view (mobile:R5). Verified: `mobile-test-client/` contains exclusively endpoint configuration, Customer CRUD, and Assistant interaction views, with zero extraneous dependencies, auth, or auxiliary databases.
- [x] 19.6 Demo hardening: README + rehearsed demo script; verify every proposal fallback rung (AI→video, collab→scripted, photo→golden, XMI→sample, assistant→canned, codegen→pre-generated, client→web/REST). Documented in `mobile-test-client/README.md` with step-by-step exam rehearsal script and complete fallback matrix covering all 7 proposal fallback rungs. Standalone demo harness `bin/demo_client.dart` verified live.
