...
describe('OpenAiVision', () => {
  ...
  it('normalizes raw LLM output with 5 classes and 13 mixed member/relation deltas without schema failure', () => {
    // Simula exactamente el caso donde un LLM emite 5 clases y 13 elementos (atributos, métodos, relaciones)
    // con formatos crudos: kind: 'attribute', op: 'create', sin memberId, visibilidad 'private', etc.
    const rawAiOutput = {
      kind: 'batch',
      id: 'batch-1',
      diagramId: 'diag-1',
      ...
      deltas: [
        // 5 Clases
        { kind: 'class', op: 'create', classId: 'c1', name: 'Usuario' },
        { kind: 'class', op: 'create', classId: 'c2', name: 'Pedido' },
        { kind: 'class', op: 'create', classId: 'c3', name: 'DetallePedido' },
        { kind: 'class', op: 'create', classId: 'c4', name: 'Producto' },
        { kind: 'class', op: 'create', classId: 'c5', name: 'Categoria' },

        // 13 elementos (deltas.5 hasta deltas.17)
        // Miembros crudos con kind: 'attribute' o kind: 'member' pero op: 'create', sin memberId
        { kind: 'attribute', classId: 'Usuario', name: 'id', dataType: 'int', visibility: 'private' },
        { kind: 'attribute', classId: 'Usuario', name: 'email', type: 'string', visibility: 'public' },
        { kind: 'method', classId: 'Usuario', name: 'autenticar(token: string)', returnType: 'boolean', visibility: 'public' },
        { kind: 'member', op: 'create', classId: 'Pedido', name: 'fecha', type: 'Date', memberType: 'attribute' },
        { kind: 'member', op: 'create', classId: 'Pedido', name: 'total', type: 'double', visibility: '-' },
        { kind: 'method', classId: 'Pedido', name: 'calcularTotal()', visibility: '+' },
        { kind: 'attribute', classId: 'DetallePedido', name: 'cantidad', type: 'int' },
        { kind: 'attribute', classId: 'DetallePedido', name: 'precioUnitario', type: 'double' },
        { kind: 'attribute', classId: 'Producto', name: 'nombre', type: 'string' },
        { kind: 'attribute', classId: 'Categoria', name: 'descripcion', type: 'string' },
        // Relaciones con sinónimos
        { kind: 'relation', source: 'Usuario', target: 'Pedido', sourceMultiplicity: '1', targetMultiplicity: '0..*' },
        { kind: 'composition', source: 'Pedido', target: 'DetallePedido', sourceMultiplicity: '1', targetMultiplicity: '1..*' },
        { kind: 'association', source: 'DetallePedido', target: 'Producto', sourceMultiplicity: '*', targetMultiplicity: '1' },
      ],
    };

    const normalized = normalizeBatchPayload(rawAiOutput);
    const parsed = BatchDeltaSchema.safeParse(normalized);
    expect(parsed.success).toBe(true);

    if (parsed.success) {
      expect(parsed.data.deltas).toHaveLength(18); // 5 clases + 10 miembros + 3 asociaciones
      const classes = parsed.data.deltas.filter((d) => d.kind === 'class');
      const members = parsed.data.deltas.filter((d) => d.kind === 'member');
      const assocs = parsed.data.deltas.filter((d) => d.kind === 'association');
      expect(classes).toHaveLength(5);
      expect(members).toHaveLength(10);
      expect(assocs).toHaveLength(3);

      // Verificar que todos los miembros tienen IDs y ops válidos
      for (const m of members) {
        expect(m.op).toMatch(/^(addAttribute|addMethod)$/);
  ...
  it('extracts nested attributes and methods from class definitions', () => {
    const rawWithNested = {
      kind: 'batch',
      deltas: [
        {
          ...
          name: 'Cliente',
          attributes: [
            { name: 'rut', type: 'string', visibility: '-' },
    ...
    const normalized = normalizeBatchPayload(rawWithNested);
    const parsed = BatchDeltaSchema.safeParse(normalized);
    expect(parsed.success).toBe(true);

    if (parsed.success) {
      expect(parsed.data.deltas).toHaveLength(4); // 1 clase + 2 atributos + 1 método = 4 total
      const classes = parsed.data.deltas.filter((d) => d.kind === 'class');
      const members = parsed.data.deltas.filter((d) => d.kind === 'member');
      expect(classes).toHaveLength(1);
  ...
  it('safely discards invalid unrecognized other deltas without failing the batch', () => {
    const rawWithJunk = {
      kind: 'batch',
      deltas: [
        { kind: 'class', op: 'create', classId: 'c1', name: 'Item' },
        { kind: 'comment', text: 'This diagram was generated by an AI' },
        { kind: 'note', text: 'Important: verify relationships' },
      ],
    };

    const normalized = normalizeBatchPayload(rawWithJunk);
    const parsed = BatchDeltaSchema.safeParse(normalized);
    expect(parsed.success).toBe(true);

    if (parsed.success) {
      expect(parsed.data.deltas).toHaveLength(1);
      expect(parsed.data.deltas[0]?.kind).toBe('class');
    }
  });
});
